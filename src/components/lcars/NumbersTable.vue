<template>
  <div class="numbers" ref="container" :data-color-scheme="colorScheme">
    <div
      v-for="(row, index) in numbers"
      :key="index"
      class="numbers-row hidden"
    >
      <span
        v-for="(item, rindex) in row"
        :key="rindex"
        class="numbers-cell"
        :class="{
          superlong: lengths[rindex] > 16,
          ...
          invisible:
            lengths[rindex] === 0 ? Math.random() > 0.66 : Math.random() < 0.02,
          'is-dot': lengths[rindex] === 0,
        }"
        :style="{
          width: lengths[rindex] * 7 + 10 + 'px',
        }"
      >
        {{ item }}
      </span>
...
function generateData() {
  ...
  const selection = [
    6, 4, 3, 10, 2, 5, 4, 2, 4, 8, 5, 2, 3, 6, 5, 7, 5, 3, 0, 6, 4, 3, 10, 2, 5,
    4, 2, 4, 8, 5, 2, 3, 6, 5, 7, 5, 3, 0, 6, 4, 3, 10, 2, 5, 4, 2, 4, 8, 5, 2,
    3, 6, 5, 7, 5, 3, 32,
  ];
  ...
  methods: {
    animate() {
      ...
      window.setTimeout(() => {
        ...
        const totalTime =
          prevDelay + randomWait + prevHighlightDelay + randomBuffer;
